...
const UpdateStudentDialog = ({
  ...
  const disabilitiesRequired =
    [dyslexia, dysgraphia, dyscalculia].filter((v) => v).length < 1;
  ...
  const handleUpdateStudents = async (e) => {
    ...
    if (
      updateFname.length < 2 ||
      updateLname.length < 2 ||
      updateUsername.length < 2
    ) {
  ...
  return (
    <Dialog
      ...
      disableAutoFocus
      sx={{ filter: archiveMode ? "grayscale(1)" : "" }}
    >
      <form onSubmit={handleUpdateStudents}>
        <DialogTitle variant="h6" bgcolor="primary.main" color="#FFF">
          Details
        </DialogTitle>
        <Divider />
        <DialogContent>
          ...
          <Box display={"flex"} gap={1} alignItems={"center"}>
            ...
            <Typography fontSize="small" sx={{ color: "primary.main" }}>
              username and password will be used to login to the PPPKids game.
            </Typography>
          </Box>

...
const Navbar = ({ navOpen }) => {
    ...
    const { auth } = useAuth();
    const isAdmin = Boolean(auth?.roles?.find(role => role === ROLES_LIST.SuperAdmin))
                ...
                <NavLink to="/" className={navOpen ? 'open' : ''}>
                    {
                        isAdmin ?
                            ...
                            <>
                                ...
                                <Typography component={'span'} className={navOpen ? 'active' : ''}>Dashboard</Typography>
                            </>
                    }
                </NavLink>
                ...
                {!isAdmin
                    ...
                    <>
                        <NavLink to="clients" className={navOpen ? 'open' : ''}>
                            <PiUserList size={26} />
                            <Typography component={'span'} className={navOpen ? 'active' : ''}>Clients</Typography>
                        </NavLink>
                        <NavLink to="violations" className={navOpen ? 'open' : ''}>
                            <RiFolderWarningLine size={26} />
                            <Typography component={'span'} className={navOpen ? 'active' : ''}>Violations</Typography>
                        </NavLink>
                    </>
